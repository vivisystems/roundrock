...
/* ROUNDROCK BEGIN */
	if(key_file && strcmp(key_file, "-")) {
		log_dbg("Attempting to decrypt key [%d]\n", *passLen);

		if (decrypt(key_file, *key, *passLen, passLen, 0) == 0) {
			log_dbg("Key successfully decrypted (%d) [%s]\n", *passLen, *key);
		}
		else {
			log_err(cd, "Failed to decrypt key from file %s.\n", key_file);
			goto out_err;
		}
	}
/* ROUNDROCK END */
