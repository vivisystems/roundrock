  ...
  get schemaVersion() { return this.dataConnection.schemaVersion;	},
    ...
    try {
	    ...
	    if (sSetting == "temp_store_directory")
	      ...
	      alert("PRAGMA " + sSetting + ": exception - " + e.message);
	  }
	},
...
//	this.aColumns = new Array();
  ...
  // selectQuery11 : execute a select query and store the results
  selectQuery11: function(sQuery, bBlobAsHex) {
    this.aTableData = new Array();
    this.aTableType = new Array();
// if aColumns is not null, there is a problem in tree display
//	this.aColumns = new Array();
    ...
    try {		// mozIStorageStatement
			...
			// statement will be undefined because it throws error);
			this.onSqlError(e, "createStatement failed: " + sQuery, 
			  		this.dataConnection.lastErrorString);
			this.setErrorString();
			return false;
		}
  ...
  getIndexInfo: function(sIndexName, iKind) {
		//to find the table and sql of the index; there are no master indexes, so the last arg will always be 0 or 2
