 ...
 * @param {String} charset          This is the charset of input/output stream
 */
GeckoJS.File.prototype.open = function(aMode, aPerms, charset) {
	...
	if(aMode.indexOf("r") != -1) {
        ...
        if(charset) {
            ...
            is.init(fis, charset, 1024, Components.interfaces.nsIConverterInputStream.DEFAULT_REPLACEMENT_CHARACTER);
        ...
        if(charset) {
            ...
            os.init(fos, charset, 1024, 0x0000);

            this._InputStream = os;
        }else {
            this._OutStream = fos;
        }
 ...
 */
GeckoJS.File.prototype.readline = function () {
    ...
    try {
      ...
      var line = {};
      var cnt = false
      if (this._InputStream instanceof Components.interfaces.nsIUnicharLineInputStream) {

        cnt = this._InputStream.readLine(line);
        return line.data;
        
      }else if (this._InputStream instanceof Components.interfaces.nsILineInputStream) {
 ...
 */
GeckoJS.Datasource.prototype.execute = function (sql, params) {
    return false;
};
...
GeckoJS.BaseModel.prototype.schema = function (recursive) {
    ...
    this._associations.forEach(function(type) {
        ...
        for(var i=0; i<self[type].length; i++) {
            ...
            if (type == 'belongsTo') {
                schema.indexes.push(schema.associations[type][name]['foreignKey']);
            }
        }
...
GeckoJS.BaseModel.prototype.getFieldsInfo = function(field) {
    ...
    if (this._fieldsinfo == null || field === true) {
        ...
        this._fieldsinfo = ds.describe(this, field);
    }
...
GeckoJS.BaseModel.prototype.find = function(type, params) {
    ...
    try {
        ...
        if (beforeResult) {
            ...
            switch(type) {
                case 'all':
                    result = ds.querySelect(this, fields, conditions, order, limit, page, recursive);
                    // if(result.length == 0) result = null;
                    break;
				
                case 'first':
                    result = ds.querySelect(this, fields, conditions, order, 1, 1, recursive);
                    if(result.length == 0) result = null;
...
GeckoJS.BaseModel.prototype.findByIndex = function(type, params){
    ...
    try {
        ...
        if (beforeResult) {
            ...
            switch(type) {
                case 'all':
                    result = ds.querySelectByIndex(this, fields, index, value, order, limit, page, recursive );
                    //if(result.length == 0) result = null;
                    break;
				
                case 'first':
                    result = ds.querySelectByIndex(this, fields, index, value, order, 1, 1, recursive);
                    if(result.length == 0) result = null;
        ...
        if (result) {
            ...
            if (!this.getId()) {
                ...
                if (typeof this.data[this.primaryKey] == 'undefined' || this.data[this.primaryKey].length == 0 ) {
                    this.data[this.primaryKey] = GeckoJS.String.uuid();
                }
...
GeckoJS.BaseModel.prototype.del = function(id, cascade){
    ...
    try {
        ...
        if (beforeResult) {
            ...
            var result = ds.executeDelete(this);
...
GeckoJS.BaseModel.prototype.escapeField = function(field, alias) {
    ...
    if (typeof ds.name == 'function') return ds.name(alias + '.' + field);
 ...
 * @return {Object}                  The result set
 */
GeckoJS.DatasourceJsonFile.prototype.querySelect = function(model, fields, conditions, order, limit, page, recursive){
 ...
 * @return {Object}                  The result set
 */
GeckoJS.DatasourceJsonFile.prototype.querySelectByIndex = function(model, fields, index, value, order, limit, page, recursive){

    recursive = typeof recursive != 'undefined' ? recursive : 1;
    
    // sdk for json not support quick index.
    var conditions = index + " = '" + value + "'";
    return this.querySelect(model, fields, conditions, order, limit, page, recursive);
 ...
 * @return {Object}                  The result set
 */
GeckoJS.DatasourceSQL.prototype.querySelect = function(model, fields, conditions, order, limit, page, recursive){
    ...
    try {
        ...
        this.log('TRACE', 'querySelect > queryData: ' + this.dump(queryData));
        ...
        this.log('TRACE', "querySelect > before fetchAll: " + query);
...
GeckoJS.DatasourceSQL.prototype._generateAssociationQuery = function(model, assocModel, type, assocName, assocSchema, queryData) {
    ...
    if (assocModel == null) {
        ...
        this.log('TRACE', '_generateAssociationQuery >  buildStatement queryData: ' + this.dump(queryData));
    ...
    switch(type) {
        ...
        case 'hasMany':
            ...
            assocFields = this.fields(assocModel);
            assocConditions = this.name(assocSchema.table+'.'+schema.foreignKey) + ' IN ( {__$FOREIGN_IDS$__} ) ';
...
GeckoJS.DatasourceSQL.prototype._queryAssociations = function(model, assocModel, type, assocName, assocData, queryData, result, recursive) {
    ...
    if (count >0) {
        ...
        if (type == 'hasMany' && assocData['primaryKey']) {

            var queryData2 = {fields: [model.primaryKey]};
            ...
            for (var i = 0; i < count; i++) {
                // ids= $in;
                var id = result[i][model.primaryKey].toString();
                idmaps[id] = i;
                ids.push(id);
            }
            ...
            this.log('TRACE', '_queryAssociations >  fetchAll: ' + query);
...
GeckoJS.DatasourceSQL.prototype._mergeHasMany = function(result, assocResults, model, assocModel, assocName, assocData, idmaps)  {
    ...
    assocResults.forEach(function(assoc) {
        ...
        var foreignId = d[modelSchema.foreignKey];
        var resultIndex = idmaps[foreignId];

        if(typeof resultIndex == 'undefined' || !result[resultIndex]) return;
 ...
 * @return {Number}                 The number of rows in the result set
 */
GeckoJS.DatasourceSQL.prototype.querySelectCount = function(model, conditions, order, limit, page){

    var result = this.querySelect(model, "COUNT(*) AS count", conditions, order, limit, page, 0);
...
GeckoJS.DatasourceSQL.prototype.querySelectById = function(model, fields, id, recursive){
    ...
    var result = this.querySelect(model, fields, conditions, null, 1, 1, recursive);
    if(result && result.length > 0) {
        return result[0];
    }
    return null;
 ...
 * @return {Number}                 The number of rows in the result set
 */
GeckoJS.DatasourceSQL.prototype.querySelectCountByIndex = function(model, index, value, order, limit, page){

    var result = this.querySelectByIndex(model, "COUNT(*) AS count", index, value, order, limit, page, 0);
    if(result && result.length > 0) {
        return result[0]['count'];
    }
    return 0;
 ...
 * @return {Object}                  The result set
 */
GeckoJS.DatasourceSQL.prototype.querySelectByIndex = function(model, fields, index, value, order, limit, page, recursive){
    ...
    var result = this.querySelect(model, fields, conditions, order, limit, page, recursive);
 ...
 * @function
 */
GeckoJS.DatasourceSQL.prototype.begin = function()	{
 ...
 * @function
 */
GeckoJS.DatasourceSQL.prototype.commit = function() {
 ...
 * @function
 */
GeckoJS.DatasourceSQL.prototype.rollback = function() {
...
GeckoJS.DatasourceSQL.prototype.fetchAll = function (sql, params, cache, model) {
    ...
    if (this.execute(sql, params)) {
        ...
        this.log('TRACE', 'fetchAll > executed ' + sql);
        ...
        this.log('TRACE', 'fetchAll > failure: ' + sql);
        return false;
    }
};
 ...
 */
GeckoJS.DatasourceSQL.prototype.execute = function (sql, params) {
...
GeckoJS.DatasourceSQL.prototype.defaultConditions = function (model, conditions) {
    ...
    return table+'.'+model.primaryKey +"='"+ model.id+"'";
...
GeckoJS.DatasourceSQL.prototype.fields = function(model, fields, quote) {
    ...
    if (fcount >= 1 && (fields2[0].indexOf('*') == -1 && fields2[0].indexOf('COUNT') == -1) ) {
        ...
        for (var i = 0; i < fcount; i++) {
            ...
            // has sql function in statement or already has AS 
            if (fields2[i].match(/^.+\(.*\)/) || fields2[i].match(/\x20AS\x20/i) ) {


            }else if (fields2[i].indexOf('.') != -1) {
 ...
 * @return {Boolean}
 */
GeckoJS.DatasourceSQLite.prototype._execute = function(sql, params) {
    ...
    try {
        if (sql.match(/^\s*select|^\s*pragma/i)) {
        ...
        this.log('ERROR', 'ERROR _execute ' + e.message);
        this._result = false;
    }
    return this._result;
...
GeckoJS.DatasourceSQLite.prototype.describe = function(model) {
    ...
    this.log('TRACE', 'PRAGMA table_info(' + table + ')' + this.dump(fields));
 ...
 * @function
 */
GeckoJS.DatasourceSQLite.prototype.begin = function()	{
 ...
 * @function
 */
GeckoJS.DatasourceSQLite.prototype.commit = function() {
 ...
 * @function
 */
GeckoJS.DatasourceSQLite.prototype.rollback = function() {
