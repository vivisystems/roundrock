 ...
 */
        ...
        for ($i = 0; $i < 3; $i++) {
            try {
                ...
                if (is_object($this->pdo_statement)) {
                    try {
                        $col_count = $this->pdo_statement->columnCount();
                        if ($col_count > 0) {
                            $this->cols_meta = array();
                            for ($ci=0; $ci < $col_count; $ci++) {
                                $this->cols_meta[$ci] = $this->pdo_statement->getColumnMeta($ci);
                            }
                        }else {
                            $this->cols_meta = array();
                        }
                    }catch(Exception $e) {
                    // maybe update / insert statement.
                        $this->cols_meta = array();
                    }
    ...
    function fetchResult() {
        ...
        if (count($this->rows)) {
            ...
            #pr($index);
                $value = $this->convertAsDeclType($index, $field);
                if (isset($this->map[$index]) and $this->map[$index] != "") {
                #echo "asdf: ".$this->map[$index];
                    list($table, $column) = $this->map[$index];
                    $resultRow[$table][$column] = $value; //$row[$index];
                } else {
                    $resultRow[0][str_replace('"', '', $index)] = $value ; //$row[$index];
                }
                $i++;
            }
            #pr($resultRow);
    ...
    function convertAsDeclType($index, $value) {

        if(empty($this->cols_meta[$index])) return $value;

        $type = $this->cols_meta[$index];

        if (empty($type['sqlite:decl_type'])) return $value;

        $declType = $type['sqlite:decl_type'];
