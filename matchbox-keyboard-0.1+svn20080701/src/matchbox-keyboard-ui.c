...
get_desktop_area(MBKeyboardUI *ui, int *x, int *y, int *width, int *height)
{
  ...
  if (x) *x           = geometry[0];
  if (y) *y           = geometry[1];
  if (width)  *width  = (ui->kbd->max_width)? ui->kbd->max_width : geometry[2]; // force to max_width and resize ui   rack lin 
  if (height) *height =  geometry[3] ; //(ui->kbd->max_height)? ui->kbd->max_height : geometry[3]; // force to max_height and resize ui   rack lin 
...
void
mb_kbd_ui_position(MBKeyboardUI  *ui)
{
...
//  ui->visible = True;
    DBG("x_win height: %d , display hegiht: %d \n", mb_kbd_ui_x_win_height(ui), mb_kbd_ui_display_height(ui));
    DBG("x_win width: %d , display width: %d \n", mb_kbd_ui_x_win_width(ui), mb_kbd_ui_display_width(ui));
    DBG("x_win x: %d , x_win y: %d \n", ui->xwin_x, ui->xwin_y);

    int dpy_height = (ui->kbd->max_height)? ui->kbd->max_height : mb_kbd_ui_display_height(ui);

    if (ui->xwin_y == 0) {
        ui->xwin_y = dpy_height - mb_kbd_ui_x_win_height(ui);
    }else {
        ui->xwin_y = 0;
    }
    XMoveWindow(ui->xdpy, ui->xwin, ui->xwin_x, ui->xwin_y);
...
#define MWM_DECOR_BORDER               (1L << 1)
  ...
  Atom /* atom_wm_protocols[3], */ 
     ...
     atom_NET_WM_WINDOW_TYPE_MESSAGE,
  ...
  */
  ...
  atom_NET_WM_WINDOW_TYPE_MESSAGE =
    XInternAtom(ui->xdpy, "_NET_WM_WINDOW_TYPE_MESSAGE", False);
      ...
      if (have_ewmh_wm)
	{
	  ...
	  if (get_desktop_area(ui, NULL, &desk_y, &desk_width, &desk_height))
	      ...
	      if (desk_width > ui->xwin_width)
		{
                  ...
                  if (ui->kbd->hfactor != 0) {
                    if (desk_height != 0) win_height = desk_height * ui->kbd->hfactor / 100; 
                    else win_height = ui->kbd->max_height * ui->kbd->hfactor / 100;
                  }else {
                    win_height = ( desk_width * ui->xwin_height ) / ui->xwin_width;
		  }
		  DBG("desk_height = %d, win_height = %d , factor = %d \n", desk_height, win_height, ui->kbd->hfactor);
		  DBG("org win_height = %d, win_height = %d \n", ( desk_width * ui->xwin_height ) / ui->xwin_width, win_height);
	      ...
	      DBG("desk width: %i, desk height: %i,xwin_width : %i, xwin_height :%i",
		  desk_width, desk_height, ui->xwin_width, ui->xwin_height);
	      DBG("desk y: %i, wm[2]: %i",  desk_y, wm_struct_vals[2]);
	  ...
	  if (have_matchbox_wm)
	    {
	      XChangeProperty(ui->xdpy, ui->xwin, 
			      ...
			      PropModeReplace, 
			      (unsigned char *) /*&atom_NET_WM_WINDOW_TYPE_TOOLBAR*/&atom_NET_WM_WINDOW_TYPE_MESSAGE, 1);
	    }
	  else
	    {
	      /*
	  ...
	  // move to button
	  DBG("mb_kbd_ui_position(ui);");
	  mb_kbd_ui_position(ui);
