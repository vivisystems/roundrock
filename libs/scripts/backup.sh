...
# set limit to 7 if not big disk
local_limit=2
bigdisk_marker=/data/.bigdisk
license_file=/etc/vivipos.lic
if [ -r "$bigdisk_marker" -a -r "$license_file" ]
then
    cmp -s -i37:0 "$bigdisk_marker" "$license_file"
    if [ $? -eq 0 ]
    then
        local_limit=7
    fi
...
do_backup() {
	...
	for tmp_dir in $all_bak_dirs; do
		...
		if [ -z $alpha_dir ]; then 
			idx=$((idx+1))
			if  `test $idx -gt $local_limit` ; then
			rm -fr "$bak/$tmp_dir"
			fi
		fi
	done
	...
	echo "40\n# Backup 'databases'"
	cd /data/databases;
	tar cjvpf $bak/$bak_dir/databases.tbz . | sed -e "s/.*/\"\0\"/g" | xargs -L1 basename | sed "s/.*/40\n# \0/g"

	# backup profile
	if [ "$backup_with_profile" = "with-profile" ]; then
	echo "50\n# Backup 'profile'"
	cd /data/profile
	tar cjvpf $bak/$bak_dir/profile.tbz --exclude="./chrome/userChrome.css" --exclude="./chrome/userConfigure.js" . | sed -e "s/.*/\"\0\"/g" | xargs -L1 basename | sed "s/.*/50\n# \0/g"
	fi
	...
	tar cjvpf $bak/$bak_dir/system.tbz . | sed -e "s/.*/\"\0\"/g" | xargs -L1 basename | sed "s/.*/70\n# \0/g"
