(function(){
    ...
    var __controller__ = {
            ...
            if (!shift) {
                ...
                if (parseInt(shiftMarkerModel.lastError) != 0) {
                    this._dbError(shiftMarkerModel.lastError, shiftMarkerModel.lastErrorString,
                                  _('An error was encountered while retrieving shift change configuration (error code %S) [message #1401].', [shiftMarkerModel.lastError]));
                    return;
                }
            }
        ...
        _setShift: function(salePeriod, shiftNumber, endOfPeriod, endOfShift) {
            ...
            if (!r) {
                this._dbError(shiftMarkerModel.lastError, shiftMarkerModel.lastErrorString,
                              _('An error was encountered while updating shift change configuration (error code %S) [message #1402].', [shiftMarkerModel.lastError]));
            }
            else {
                // update shift
        ...
        truncateData: function(evt) {
            try {
                ...
                if (!r) {
                    throw {errno: model.lastError,
                           errstr: model.lastErrorString,
                           errmsg: ('An error was encountered while removing shift change marker (error code %S) [message #1409].', [model.lastError])};
                }
            }
            catch(e) {
                this._dbError(e.errno, e.errstr, e.errmsg);
