...
"","${_( '(rpt)Gross Sales' )}","${body.sales_summary.ItemSubtotal|default:0|viviFormatPrices:true}"
...
"","${_( '(rpt)Net Sales' )}","${body.sales_summary.Total|default:0|viviFormatPrices:true}"
"",""
"${_( '(rpt)Sales Summary' )}"
"","${_( '(rpt)Number of Orders' ) + ':'}","${body.sales_summary.OrderNum|default:0|format:0}"
"","${_( '(rpt)Net Sales' ) + ':'}","${body.sales_summary.Total|default:0|viviFormatPrices:true}"
"","${_( '(rpt)Number of Guests' ) + ':'}","${body.sales_summary.Guests|default:0|format:0}"
"","${_( '(rpt)Number of Items' ) + ':'}","${body.sales_summary.ItemsCount|default:0|format:0}"
"","${_( '(rpt)Number of Voided Orders' ) + ':'}","${body.sales_summary.VoidedOrders|default:0|format:0}"
"","${_( '(rpt)Net Sales/Order' ) + ':'}","${body.sales_summary.AvgTotal|default:0|viviFormatPrices:true}"
"","${_( '(rpt)Net Sales/Guest' ) + ':'}","${body.sales_summary.AvgTotalPerGuest|default:0|viviFormatPrices:true}"
"","${_( '(rpt)Number of Guests/Order' ) + ':'}","${body.sales_summary.AvgGuests|default:0|format:2}"
"","${_( '(rpt)Number of Items/Order' ) + ':'}","${body.sales_summary.AvgItemsCount|default:0|format:2}"
"",""
"${_( '(rpt)Payment List' )}"
"","${_( '(rpt)Payment' )}","${_( '(rpt)Amount' )}"
...
"",""
"${_( '(rpt)Hourly Sales' )}"
"","${_( '(rpt)Time' )}","${_( '(rpt)Number of Guests' )}","${_( '(rpt)Number of Orders' )}","${_( '(rpt)Total' )}"
...
"","${_( '(rpt)Name' )}","${_( '(rpt)Code' )}","${_( '(rpt)Count' )}","${_( '(rpt)Discount' )}"
{for detail in body.promotion_summary.results}
"","${detail.name}","${detail.code}","${detail.matched_count|default:0}","${detail.discount_subtotal|default:0|viviFormatPrices:true}"
{/for}
