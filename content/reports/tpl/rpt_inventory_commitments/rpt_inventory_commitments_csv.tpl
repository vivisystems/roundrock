...
"${product.product_no}","${product.name}","${product.barcode}","${product.warehouse}","${product.clerk}","${product.quantity|format:0}","${product.new_quantity|format:0}",{if commitment.type == "procure"}"${product.price|default:0|viviFormatPrices:true}"{else}""{/if},"${product.memo}"
{/for}
