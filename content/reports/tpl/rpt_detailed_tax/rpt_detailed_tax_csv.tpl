...
"${_( '(rpt)Sequence' )}","${_( '(rpt)Time' )}","${_( '(rpt)Total' )}","${_( '(rpt)Discount' )}","${_( '(rpt)Promotion' )}","${_( '(rpt)Surcharge' )}","${_( '(rpt)Add-on Tax' )}","${_( '(rpt)Included Tax' )}"{for tax in taxList},"${tax.no}"{/for}
{for item in body}
,""
"${item.Order.sequence}","${item.Order.time|unixTimeToString}","${item.Order.total|default:0|viviFormatPrices:true}","${item.discount_subtotal|default:0|viviFormatPrices:true}","${item.Order.promotion_subtotal|default:0|viviFormatPrices:true}","${item.surcharge_subtotal|default:0|viviFormatPrices:true}","${item.Order.tax_subtotal|default:0|viviFormatPrices:true}","${item.Order.included_tax_subtotal|default:0|viviFormatPrices:true}"{for tax in taxList},"${item[ tax.no ]|viviFormatPrices:true}"{/for}
{/for}
,""
