...
{for types in body}
"${types_index}"
"","${_( '(rpt)Terminal' )}","${_( '(rpt)Clerk' )}","${_( '(rpt)Sale Period' )}","${_( '(rpt)Shift' )}","${_( '(rpt)Time' )}","${_( '(rpt)Sequence' )}","${_( '(rpt)Status' )}","${_( '(rpt)Annotation Text' )}","${_( '(rpt)Quantities Sold' )}","${_( '(rpt)Gross Sales' )}","${_( '(rpt)Surcharge' )}","${_( '(rpt)Discount' )}","${_( '(rpt)Promotion' )}","${_( '(rpt)Revalue' )}","${_( '(rpt)Add-on Tax' )}","${_( '(rpt)Total' )}","${_( '(rpt)Payment' )}"
...
{/eval}
"","'${order.terminal_no}","'${order.service_clerk_displayname|default:''}/${order.proceeds_clerk_displayname|default:''}","${order.sale_period|unixTimeToString:'saleperiod'}","${order.shift_number}","${order.time|unixTimeToString}","'${order.sequence}","'${order.status}","'${order.text}","${order.qty_subtotal|viviFormatPrices:true}","${order.item_subtotal|viviFormatPrices:true}","${order.surcharge_subtotal|viviFormatPrices:true}","${order.discount_subtotal|viviFormatPrices:true}","${order.promotion_subtotal|viviFormatPrices:true}","${order.revalue_subtotal|viviFormatPrices:true}","${order.tax_subtotal|viviFormatTaxes:true}","${order.total|viviFormatPrices:true}","${order.payment|viviFormatPrices:true}"
{/for}
{eval}
