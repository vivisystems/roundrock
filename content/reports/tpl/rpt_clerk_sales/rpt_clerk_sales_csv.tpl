...
${clerk.name}
""
"${_( '(rpt)Terminal' )}","${_( '(rpt)Time' )}","${_( '(rpt)Sequence' )}","${_( '(rpt)Total' )}","${_( '(rpt)Add-on Tax' )}","${_( '(rpt)Surcharge' )}","${_( '(rpt)Discount' )}","${_( '(rpt)Promotion' )}","${_( '(rpt)Revalue' )}","${_( '(rpt)Payment' )}","${_( '(rpt)Cash' )}","${_( '(rpt)Check' )}","${_( '(rpt)Credit Card' )}","${_( '(rpt)Coupon' )}","${_( '(rpt)Gift Card' )}"
...
{/eval}
"${order.terminal_no}","${order.time|unixTimeToString}","${order.sequence}","${order.item_subtotal|default:0|viviFormatPrices:true}","${order.tax_subtotal|default:0|viviFormatTaxes:true}","${order.surcharge_subtotal|default:0|viviFormatPrices:true}","${order.discount_subtotal|default:0|viviFormatPrices:true}","${order.promotion_subtotal|default:0|viviFormatPrices:true}","${order.revalue_subtotal|default:0|viviFormatPrices:true}","${order.total|default:0|viviFormatPrices:true}","${order.cash|default:0|viviFormatPrices:true}","${order.check|default:0|viviFormatPrices:true}","${order.creditcard|default:0|viviFormatPrices:true}","${order.coupon|default:0|viviFormatPrices:true}","${order.giftcard|default:0|viviFormatPrices:true}"
{/for}
{eval}
