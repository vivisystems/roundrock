...
"${order.terminal_no}",{if clerk.associated_clerk == 'Proceeds Clerk'}"${order.proceeds_clerk_displayname}",{/if}{if clerk.associated_clerk == 'Service Clerk'}"${order.service_clerk_displayname}",{/if}"${order.transaction_created|unixTimeToString}","${order.sequence}","${order.item_subtotal|viviFormatPrices:true}","${order.tax_subtotal|viviFormatPrices:true}","${order.surcharge_subtotal|viviFormatPrices:true}","${order.discount_subtotal|viviFormatPrices:true}","${order.total|viviFormatPrices:true}","${order.cash|default:0|viviFormatPrices:true}","${order.check|default:0|viviFormatPrices:true}","${order.creditcard|default:0|viviFormatPrices:true}","${order.coupon|default:0|viviFormatPrices:true}","${order.giftcard|default:0|viviFormatPrices:true}"
{/for}
