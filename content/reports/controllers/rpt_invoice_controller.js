(function(){
    ...
    GeckoJS.Controller.extend( {
        ...
        _set_datas: function( start, end, periodType, shiftNo, terminalNo ) {
            ...
            if ( shiftNo.length > 0 )
            	conditions += " AND orders.shfit_number = " + shiftNo;
            ...
            // for querying uniform_invoices table.
            fields = [
            			'created',
            			'start_seq',
            			'end_seq',
            			'code',
            			'uniform_business_number',
            			'order_id',
        				'shift_number',
        				'status'
        			 ];
        			 
        	if ( periodType == 'transaction_created' ) periodType = 'created';
        	conditions = "uniform_invoices." + periodType + ">='" + start +
                            "' AND uniform_invoices." + periodType + "<='" + end + "'";
                            
            if ( shiftNo.length > 0 )
            	conditions += " AND uniform_invoices.shfit_number = " + shiftNo;
            
            if ( terminalNo.length > 0 )
                conditions += " AND uniform_invoices.terminal_no LIKE '" + terminalNo + "%'";
                
            var uniformInvoice = new UniformInvoiceModel();
            
            var invoiceRecords = uniformInvoice.find( 'all', { fields: fields, conditions: conditions, recursive: 0 } );
            
            // for querying oreder_additions
			...
			invoiceRecords.forEach( function( invoiceRecord ) {
				switch ( parseInt( invoiceRecord.status ) ) {
					case 1:
						invoiceRecord.status = '';
						break;
					case 2:
						invoiceRecord.status = 'discarded';
						break;
					case -1:
						invoiceRecord.status = 'cancled';
						break;
				}
				
				var num_invoices = parseInt( invoiceRecord.end_seq ) - parseInt( invoiceRecord.start_seq ) + 1;
				summary.num_invoices += num_invoices;
				
				invoiceRecord.created = ( new Date( invoiceRecord.created * 1000 ) ).toString( 'yyyy/MM/dd' );
				
				if ( invoiceRecord.status == '' ) {	
					for ( oid in records ) {
						if ( invoiceRecord.order_id == oid ) {
							records[ oid ][ 'created' ] = invoiceRecord.created;
							records[ oid ][ 'invoice_no' ] = invoiceRecord.code + invoiceRecord.start_seq;
							records[ oid ][ 'num_invoices' ] = num_invoices;
							records[ oid ][ 'uniform_business_number' ] = invoiceRecord.uniform_business_number;
							records[ oid ][ 'shift_number' ] = invoiceRecord.shift_number;
							records[ oid ][ 'status' ] = invoiceRecord.status;
						
							invoices.push( records[ oid ] );
						}
					}
				} else {
					invoiceRecord[ 'surcharge_subtotal' ] = 0;
					invoiceRecord[ 'discount_subtotal' ] = 0;
					invoiceRecord.Order = {};
					invoiceRecord.Order.total = 0;
					invoiceRecord.Order.sequence = '';
					invoiceRecord[ 'tax_subtotal' ] = 0;
					invoiceRecord[ 'included_tax_subtotal' ] = 0;
					
					invoiceRecord[ 'invoice_no' ] = invoiceRecord.code + invoiceRecord.start_seq;
					invoiceRecord[ 'num_invoices' ] = num_invoices;

					taxList.forEach( function( tax ) {
						if ( tax.type != typeCombineTax )
							invoiceRecord[ tax.no ] = 0;
					} );
		...
		printDetailedTaxReport: function( start, end, periodType, shiftNo, terminalNo ) {
